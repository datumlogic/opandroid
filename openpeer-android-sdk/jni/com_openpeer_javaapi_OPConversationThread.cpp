		...
		jobject metadata)
{
	...
	ContactProfileInfoList coreAddContacts = ContactProfileInfoList();

	__android_log_print(ANDROID_LOG_DEBUG, "com.openpeer.jni",
			"OPConversationThread native create called");

	jclass accountClass = findClass("com/openpeer/javaapi/OPAccount");
	jfieldID accountFid = jni_env->GetFieldID(accountClass, "nativeClassPointer",
			"J");
	...
	if (jni_env)
	{
		...
		if (jni_env->IsInstanceOf(addContacts, arrayListClass) != JNI_TRUE)
		{
			return object;
		}
		...
		//get size of the addContacts list
		int addContactsSize = (int) jni_env->CallIntMethod(addContacts, sizeMethodID);
		//if there are elements in addContact list, add them to the list for core method
		if(addContactsSize > 0)
		{
			for( int i=0; i<addContactsSize; ++i )
			{
				// Call "java.util.List.get" method and get Contact object by index.
				jobject contactProfileInfoObject = jni_env->CallObjectMethod( addContacts, listGetMethodID, i );
				if( contactProfileInfoObject != NULL )
				{
					...
					jmethodID getIdentityContactsMethodID = jni_env->GetMethodID( contactProfileInfoClass, "getIdentityContacts", "()Ljava/util/List;" );
					...
					jobject identityContacts = jni_env->CallObjectMethod(contactProfileInfoObject, getIdentityContactsMethodID);
					//initilize identity contqcts list
					contactProfileInfo.mIdentityContacts = identityContactListFromJava(identityContacts);
					//add core contacts to list for removal
					coreAddContacts.push_front(contactProfileInfo);
				}
			}
	...
	if (conversationThreadPtr)
	{
		...
		if (jni_env)
		{
			...
			IConversationThreadPtr* ptrToConversationThread = new boost::shared_ptr<
					IConversationThread>(conversationThreadPtr);
	...
	else
	{
		__android_log_print(ANDROID_LOG_ERROR, "com.openpeer.jni",
				"OPConversationThread native create core pointer is NULL!!!");
	}
	return object;
}
...
(JNIEnv *, jclass, jobject javaAccount)
{
	...
	__android_log_print(ANDROID_LOG_DEBUG, "com.openpeer.jni",
			"OPConversationThread native getConversationThreads called");
	...
	if (coreAccountPtr)
	{
		coreConversationThreads = IConversationThread::getConversationThreads(
				*coreAccountPtr);
	}
	else
	{
		__android_log_print(ANDROID_LOG_ERROR, "com.openpeer.jni",
				"OPConversationThread native getConversationThreads account core pointer is NULL!!!");
	}
	//fetch JNI env
	jni_env = getEnv();
	if (jni_env)
	{
		...
		jmethodID listConstructorMethodID = jni_env->GetMethodID(returnListClass,
				"<init>", "()V");
		returnListObject = jni_env->NewObject(returnListClass,
				listConstructorMethodID);

		//fetch List.add object
		jmethodID listAddMethodID = jni_env->GetMethodID(returnListClass, "add",
				"(Ljava/lang/Object;)Z");

		//fill/update map
		for (ConversationThreadList::iterator coreListIter =
				coreConversationThreads->begin();
				coreListIter != coreConversationThreads->end(); coreListIter++)
		{
			//fetch List item object / OPConversationThread
			jclass conversationThreadClass = findClass(
					"com/openpeer/javaapi/OPConversationThread");
			jmethodID conversationThreadConstructorMethodID = jni_env->GetMethodID(
					conversationThreadClass, "<init>", "()V");
			jobject conversationThreadObject = jni_env->NewObject(
					conversationThreadClass, conversationThreadConstructorMethodID);

			IConversationThreadPtr* ptrToConversationThread = new boost::shared_ptr<
					IConversationThread>(*coreListIter);
			jfieldID fid = jni_env->GetFieldID(conversationThreadClass,
					"nativeClassPointer", "J");
			jlong convThread = (jlong) ptrToConversationThread;
			jni_env->SetLongField(conversationThreadObject, fid, convThread);

			//add to return List
			jboolean success = jni_env->CallBooleanMethod(returnListObject,
					listAddMethodID, conversationThreadObject);
...
(JNIEnv *, jclass, jobject javaAccount, jstring threadID)
{
	...
	__android_log_print(ANDROID_LOG_DEBUG, "com.openpeer.jni",
			"OPConversationThread native getConversationThreadByID called");
	...
	if (coreAccountPtr)
	{
		conversationThreadPtr = IConversationThread::getConversationThreadByID(
				*coreAccountPtr, threadIDStr);
	}
	else
	{
		__android_log_print(ANDROID_LOG_ERROR, "com.openpeer.jni",
				"OPConversationThread native getConversationThreadByID account core pointer is NULL!!!");
	}
	...
	{
		__android_log_print(ANDROID_LOG_ERROR, "com.openpeer.jni",
				"OPConversationThread native getConversationThreadByID core pointer is NULL!!!");
	}
	return object;
}
...
(JNIEnv *, jobject owner)
{
	...
	__android_log_print(ANDROID_LOG_DEBUG, "com.openpeer.jni",
			"OPConversationThread native getStableID called");
	...
	IConversationThreadPtr* coreConversationThreadPtr =
			(IConversationThreadPtr*) pointerValue;
	...
	{
		__android_log_print(ANDROID_LOG_ERROR, "com.openpeer.jni",
				"OPConversationThread native getStableID core pointer is NULL!!!");
	}
	return ret;
}
...
(JNIEnv *, jobject owner)
{
	...
	__android_log_print(ANDROID_LOG_DEBUG, "com.openpeer.jni",
			"OPConversationThread native getThreadID called");
	...
	if (coreConversationThreadPtr)
	{
		ret = jni_env->NewStringUTF(
				coreConversationThreadPtr->get()->getThreadID().c_str());
	}
	else
	{
		__android_log_print(ANDROID_LOG_ERROR, "com.openpeer.jni",
...
(JNIEnv *, jobject owner)
{
	...
	__android_log_print(ANDROID_LOG_DEBUG, "com.openpeer.jni",
			"OPConversationThread native amIHost called");
	...
	{
		__android_log_print(ANDROID_LOG_ERROR, "com.openpeer.jni",
				"OPConversationThread native amIHost core pointer is NULL!!!");
	}
	return ret;
}
...
(JNIEnv *, jobject owner)
{
	...
	__android_log_print(ANDROID_LOG_DEBUG, "com.openpeer.jni",
			"OPConversationThread native getAssociatedAccount called");

	jni_env = getEnv();
	jclass conversationThreadClass = findClass(
			"com/openpeer/javaapi/OPConversationThread");
	jfieldID conversationThreadFid = jni_env->GetFieldID(conversationThreadClass,
			"nativeClassPointer", "J");
	jlong conversationThreadPointerValue = jni_env->GetLongField(owner,
			conversationThreadFid);

	IConversationThreadPtr* conversationThreadPtr =
			(IConversationThreadPtr*) conversationThreadPointerValue;
	if (conversationThreadPtr)
	{
		IAccountPtr accountPtr =
				conversationThreadPtr->get()->getAssociatedAccount();
		if (accountPtr)
		{
			jclass accountClass = findClass("com/openpeer/javaapi/OPAccount");
			jmethodID accountMethod = jni_env->GetMethodID(accountClass, "<init>",
					"()V");
			...
			jfieldID fid = jni_env->GetFieldID(accountClass, "nativeClassPointer",
					"J");
	...
	{
		__android_log_print(ANDROID_LOG_ERROR, "com.openpeer.jni",
				"OPConversationThread native getAssociatedAccount core pointer is NULL!!!");
	}
...
(JNIEnv *, jobject owner)
{
	...
	__android_log_print(ANDROID_LOG_DEBUG, "com.openpeer.jni",
			"OPConversationThread native getContacts called");
	...
	{
		__android_log_print(ANDROID_LOG_ERROR, "com.openpeer.jni",
				"OPConversationThread native getContacts core pointer is NULL!!!");
	}
	...
	if (jni_env)
	{
				...
				coreListIter != coreContacts->end(); coreListIter++)
		{
			...
			jclass contactClass = findClass("com/openpeer/javaapi/OPContact");
			jmethodID contactConstructorMethodID = jni_env->GetMethodID(contactClass,
					"<init>", "()V");
			jobject contactObject = jni_env->NewObject(contactClass,
					contactConstructorMethodID);

			IContactPtr* ptrToContact = new boost::shared_ptr<IContact>(
					*coreListIter);
...
(JNIEnv *, jobject owner, jobject contact)
{
	...
	__android_log_print(ANDROID_LOG_DEBUG, "com.openpeer.jni",
			"OPConversationThread native getIdentityContactList called");
	...
	{
		coreContactList = coreConversationThreadPtr->get()->getIdentityContactList(
				*contactPtr);
		if (coreContactList->size() == 0) {
			__android_log_print(ANDROID_LOG_ERROR, "com.openpeer.jni",
					"OPConversationThread native getIdentityContactList retrieved 0!!!");
		}
	}
	else
	{
		__android_log_print(ANDROID_LOG_ERROR, "com.openpeer.jni",
				"OPConversationThread native getIdentityContactList core pointer is NULL!!!");
	}
	...
	if (jni_env)
	{
		...
		for (IdentityContactList::iterator iter = coreContactList->begin();
				iter != coreContactList->end(); iter++)
		{
			coreContact = *iter;
			...
			method = jni_env->GetMethodID(cls, "setStableID",
					"(Ljava/lang/String;)V");
			...
			//jclass dispositionClass = findClass("com/openpeer/javaapi/OPRolodexContact$Dispositions");
			//jmethodID dispositionConstructorMethodID = jni_env->GetMethodID(cls, "<init>", "()V");
			jmethodID setDispositionMethodID = jni_env->GetMethodID(cls,
					"setDisposition",
					"(Lcom/openpeer/javaapi/OPRolodexContact$Dispositions;)V");
			//Fetch setIdentityURI method from OPDownloadedRolodexContacts class
			jmethodID setIdentityURIMethodID = jni_env->GetMethodID(cls,
					"setIdentityURI", "(Ljava/lang/String;)V");
			//Fetch setIdentityProvider method from OPDownloadedRolodexContacts class
			jmethodID setIdentityProviderMethodID = jni_env->GetMethodID(cls,
					"setIdentityProvider", "(Ljava/lang/String;)V");
			//Fetch setName method from OPDownloadedRolodexContacts class
			jmethodID setNameMethodID = jni_env->GetMethodID(cls, "setName",
					"(Ljava/lang/String;)V");
			//Fetch setProfileURL method from OPDownloadedRolodexContacts class
			jmethodID setProfileURLMethodID = jni_env->GetMethodID(cls,
					"setProfileURL", "(Ljava/lang/String;)V");
			//Fetch setVProfileURL method from OPDownloadedRolodexContacts class
			jmethodID setVProfileURLMethodID = jni_env->GetMethodID(cls,
					"setVProfileURL", "(Ljava/lang/String;)V");
			//Fetch setAvatars method from OPDownloadedRolodexContacts class
			jmethodID setAvatarsMethodID = jni_env->GetMethodID(cls, "setAvatars",
					"(Ljava/util/List;)V");

			//avatar list fetch
			jclass avatarListClass = findClass("java/util/ArrayList");
			jmethodID avatarListConstructorMethodID = jni_env->GetMethodID(
					avatarListClass, "<init>", "()V");
			jobject avatarListObject = jni_env->NewObject(avatarListClass,
					avatarListConstructorMethodID);
			jmethodID avatarListAddMethodID = jni_env->GetMethodID(avatarListClass,
					"add", "(Ljava/lang/Object;)Z");

			//OPAvatar class and methods fetch
			jclass avatarClass = findClass(
					"com/openpeer/javaapi/OPRolodexContact$OPAvatar");
			jmethodID avatarConstructorMethodID = jni_env->GetMethodID(avatarClass,
					"<init>", "()V");
			jmethodID setAvatarNameMethodID = jni_env->GetMethodID(avatarClass,
					"setName", "(Ljava/lang/String;)V");
			jmethodID setAvatarURLMethodID = jni_env->GetMethodID(avatarClass,
					"setURL", "(Ljava/lang/String;)V");
			jmethodID setAvatarWidthMethodID = jni_env->GetMethodID(avatarClass,
					"setWidth", "(I)V");
			jmethodID setAvatarHeightMethodID = jni_env->GetMethodID(avatarClass,
					"setHeight", "(I)V");

			//set Disposition to OPRolodexContact
			jobject dispositionObject = OpenPeerCoreManager::getJavaEnumObject(
					"com/openpeer/javaapi/OPRolodexContact$Dispositions",
					(jint) coreContact.mDisposition);
			jni_env->CallVoidMethod(object, setDispositionMethodID,
					dispositionObject);

			//set identity URI to OPRolodexContact
			jstring identityUriStr = jni_env->NewStringUTF(
					coreContact.mIdentityURI.c_str());
			jni_env->CallVoidMethod(object, setIdentityURIMethodID, identityUriStr);

			//set identity provider to OPRolodexContact
			jstring identityProviderStr = jni_env->NewStringUTF(
					coreContact.mIdentityProvider.c_str());
			jni_env->CallVoidMethod(object, setIdentityProviderMethodID,
					identityProviderStr);
			...
			jstring profileURLStr = jni_env->NewStringUTF(
					coreContact.mProfileURL.c_str());
			jni_env->CallVoidMethod(object, setProfileURLMethodID, profileURLStr);

			//set v profile URL to OPRolodexContact
			jstring vProfileURLStr = jni_env->NewStringUTF(
					coreContact.mVProfileURL.c_str());
			jni_env->CallVoidMethod(object, setVProfileURLMethodID, vProfileURLStr);

			//set avatars to OPAvatarList
			for (RolodexContact::AvatarList::iterator avatarIter =
					coreContact.mAvatars.begin();
					avatarIter != coreContact.mAvatars.end(); avatarIter++)
			{
				RolodexContact::Avatar coreAvatar = *avatarIter;
				//create OPAvatar object
				jobject avatarObject = jni_env->NewObject(avatarClass,
						avatarConstructorMethodID);

				//set avatar name to OPRolodexContact::OPAvatar
				jstring avatarNameStr = jni_env->NewStringUTF(coreAvatar.mName.c_str());
				jni_env->CallVoidMethod(avatarObject, setAvatarNameMethodID,
						avatarNameStr);

				//set avatar URL to OPRolodexContact::OPAvatar
				jstring avatarURLStr = jni_env->NewStringUTF(coreAvatar.mURL.c_str());
				jni_env->CallVoidMethod(avatarObject, setAvatarURLMethodID,
						avatarURLStr);

				//set avatar width to OPRolodexContact::OPAvatar
				jni_env->CallVoidMethod(avatarObject, setAvatarWidthMethodID,
						(jint) coreAvatar.mWidth);

				//set avatar height to OPRolodexContact::OPAvatar
				jni_env->CallVoidMethod(avatarObject, setAvatarHeightMethodID,
						(jint) coreAvatar.mHeight);

				//add avatar object to avatar list
				jboolean success = jni_env->CallBooleanMethod(avatarListObject,
						avatarListAddMethodID, avatarObject);
			}
...
(JNIEnv *, jobject owner, jobject contact)
{
	...
	__android_log_print(ANDROID_LOG_DEBUG, "com.openpeer.jni",
			"OPConversationThread native getContactState called");
	...
	{
		state = (jint) coreConversationThreadPtr->get()->getContactConnectionState(
				*coreContactPtr);
		if (jni_env)
		{
			object = OpenPeerCoreManager::getJavaEnumObject(
					"com/openpeer/javaapi/ContactConnectionStates", state);
	...
	{
		__android_log_print(ANDROID_LOG_ERROR, "com.openpeer.jni",
				"OPConversationThread native getContactState core pointer is NULL!!!");
	}
	return object;
}
...
(JNIEnv *, jobject owner, jobject contactProfileInfos)
{
	...
	if (coreConversationThreadPtr)
	{
		...
		__android_log_print(ANDROID_LOG_DEBUG, "com.openpeer.jni", "OPConversationThread native addContacts added %d",coreContactProfilesToAdd.size());
...
(JNIEnv *, jobject owner, jobject contact)
{
	...
	if (coreConversationThreadPtr && coreContactPtr)
	{
		ElementPtr coreEl = coreConversationThreadPtr->get()->getContactStatus(
				*coreContactPtr);
		if (coreEl) {
			ElementPtr* ptrToElement = new boost::shared_ptr<Element>(coreEl);
			cls = findClass("com/openpeer/javaapi/OPElement");
			...
			__android_log_print(ANDROID_LOG_DEBUG, "com.openpeer.jni",
					"OPConversationThread getContactStatus ElementPtr null");
...
(JNIEnv *env, jobject owner, jstring messageID)
{
	...
	__android_log_print(ANDROID_LOG_DEBUG, "com.openpeer.jni",
			"OPConversationThread native getMessage called");
	...
	if (coreConversationThreadPtr)
	{
		...
		if (jni_env)
		{
			...
			jboolean exists = coreConversationThreadPtr->get()->getMessage(
					messageIDStr, outReplacesMessageID, outFrom, outMessageType,
					outMessage, outTime, outValidated);
			if (exists)
			{
				...
				jmethodID messageConstructorMethodID = jni_env->GetMethodID(
						messageClass, "<init>", "()V");
				messageObject = jni_env->NewObject(messageClass,
						messageConstructorMethodID);

				//FETCH METHODS TO GET INFO FROM JAVA
				//Fetch setMessageID method from OPMessage class
				jmethodID setMessageIdMethodID = jni_env->GetMethodID(messageClass,
						"setMessageId", "(Ljava/lang/String;)V");
				//Fetch setMessageID method from OPMessage class
				jmethodID setReplacesMessageIdMethodID = jni_env->GetMethodID(
						messageClass, "setReplacesMessageId", "(Ljava/lang/String;)V");
				//Fetch setFrom method from OPMessage class
				jmethodID setFromMethodID = jni_env->GetMethodID(messageClass,
						"setFrom", "(Lcom/openpeer/javaapi/OPContact;)V");
				//Fetch setMessageType method from OPMessage class
				jmethodID setMessageTypeMethodID = jni_env->GetMethodID(messageClass,
						"setMessageType", "(Ljava/lang/String;)V");
				//Fetch setMessage method from OPMessage class
				jmethodID setMessageMethodID = jni_env->GetMethodID(messageClass,
						"setMessage", "(Ljava/lang/String;)V");
				//Fetch setTime method from OPMessage class
				jmethodID setTimeMethodID = jni_env->GetMethodID(messageClass,
						"setTime", "(Landroid/text/format/Time;)V");
				//Fetch setValidated method from OPMessage class
				jmethodID setValidatedMethodID = jni_env->GetMethodID(messageClass,
						"setValidated", "(Z)V");
				...
				jmethodID contactConstructorMethodID = jni_env->GetMethodID(
						contactClass, "<init>", "()V");
				jobject from = jni_env->NewObject(contactClass,
						contactConstructorMethodID);
				...
				//Call setMessageId method on return object
				jstring replacesMessageID = jni_env->NewStringUTF(
						outReplacesMessageID.c_str());
				jni_env->CallVoidMethod(messageObject, setReplacesMessageIdMethodID,
						replacesMessageID);
				...
				jni_env->CallVoidMethod(messageObject, setMessageTypeMethodID,
						messageType);
				...
				jni_env->CallVoidMethod(messageObject, setValidatedMethodID,
						outValidated);
			}
		}
	}
	...
	{
		__android_log_print(ANDROID_LOG_ERROR, "com.openpeer.jni",
				"OPConversationThread native getMessage core pointer is NULL!!!");
	}
...
(JNIEnv *env, jobject owner, jstring messageID, jobject state)
{
	jclass cls;
	jmethodID method;
	jobject object;
	...
	__android_log_print(ANDROID_LOG_DEBUG, "com.openpeer.jni",
			"OPConversationThread native setMessageDeliveryState called");
	...
	if (coreConversationThreadPtr)
	{
		if (jni_env)
		{
			coreConversationThreadPtr->get()->setMessageDeliveryState(
					messageIDStr, (IConversationThread::MessageDeliveryStates)OpenPeerCoreManager::getIntValueFromEnumObject(state, "com/openpeer/javaapi/OPMessageDeliveryStates"));
		}
	}
	else
	{
		__android_log_print(ANDROID_LOG_ERROR, "com.openpeer.jni",
				"OPConversationThread native setMessageDeliveryState core pointer is NULL!!!");
	}
}
...
(JNIEnv *env, jobject owner, jstring messageID)
{
	...
	__android_log_print(ANDROID_LOG_DEBUG, "com.openpeer.jni",
			"OPConversationThread native getMessageDeliveryState called");
	...
	if (coreConversationThreadPtr)
	{
		if (jni_env)
		{
			...
			jboolean exists =
					coreConversationThreadPtr->get()->getMessageDeliveryState(
							messageIDStr, stateValue);
			if (exists)
			{
				object = OpenPeerCoreManager::getJavaEnumObject(
						"com/openpeer/javaapi/OPMessageDeliveryStates", (jint) stateValue);
			}
			else
			{
				return NULL;
	...
	{
		__android_log_print(ANDROID_LOG_ERROR, "com.openpeer.jni",
				"OPConversationThread native getMessageDeliveryState core pointer is NULL!!!");
	}
	return object;
}
